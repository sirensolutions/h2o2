...
internals.schema = Validate.object({
    ...
    mapHttpClientOptions: Validate.func()
})
    .xor('host', 'mapUri', 'uri')
    .without('mapUri', 'port')
...
internals.handler = function (route, handlerOptions) {
    ...
    return async function (request, h) {
        ...
        if (settings.mapHttpClientOptions) {
            Hoek.merge(options, await settings.mapHttpClientOptions(request));
        }

        const promise = settings.httpClient.request(request.method, internals.encodeUri(uri), options);
...
internals.encodeUri = function (uri) {

    const tempUri = URL.parse(uri);
    if (decodeURI(tempUri.pathname) === tempUri.pathname) {
