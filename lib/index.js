    ...
    return function (request, reply) {
        ...
        settings.mapUri(request, (err, uri, headers) => {
            ...
            const _encodeURL = (uri) => {
                ...
                if (uri.match(/\?./)) {
                    haveQueryParam = true;
                    uriBasePath = uri.split('?')[0];
                    queryParams = uri.split('?')[1];
                }

                if (decodeURI(uriBasePath) === uriBasePath) {
                    uriBasePath = encodeURI(uriBasePath);
                }       
                return haveQueryParam ? `${uriBasePath}?${queryParams}` : uriBasePath;
            };
