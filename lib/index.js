...
internals.schema = Joi.object({
    ...
    acceptEncoding: Joi.when('passThrough', { is: true, then: Joi.boolean().default(true), otherwise: Joi.forbidden() }),
...
internals.handler = function (route, handlerOptions) {
    ...
    handlerOptions = internals.schema.validate(handlerOptions).value;
...
internals.sendRequest = async function (request, uri, options, h, bind, settings) {
    let res;
    try {
        res = await Wreck.request(request.method, uri, options);
    } catch (e) {
        if (settings.onResponse) {
